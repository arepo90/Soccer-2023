...
//The delay system only works well with POWER < 60 in JM motors
void line(){
    if(lin3){
        bwd((lin3 == 1 ? 2 : lin3));
        if(lin3 >= 2) delay(DEL);
...
//Returning when no ball is detected
void comeback(){
    if(readLines()){
        line();
        return;
...
//Simple proportional orientation
void orientation(){
